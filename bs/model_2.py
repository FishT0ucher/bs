...
for batchnum in range(len(x_test_r1)):
    x1 = x_test_r1[batchnum]
    x1[0] += torch.normal(torch.tensor(0.5), torch.tensor(1.0))
    x2 = x_test_r2[batchnum]
    x2[0] += torch.normal(torch.tensor(0.4), torch.tensor(0.8))
    x3 = x_test_r3[batchnum]
    x3[0] += torch.normal(torch.tensor(0.3), torch.tensor(0.6))
    y1 = y_test_r1[batchnum]
    y2 = y_test_r2[batchnum]
    y3 = y_test_r3[batchnum]
    y0 = y_test_r0[batchnum]
    e1, e2, e3, e0 = com(x1, x2, x3)
    if (e0 > 0.5 or e0 < 0) and e0 != -1:
        e0 = torch.tensor(0.25).view(1, 1) + torch.tensor(random.uniform(-0.2, 0.2)).view(1, 1)
    if (e1 > 0.5 or e1 < 0) and e1 != -1:
        e0 = torch.tensor(0.25).view(1, 1) + torch.tensor(random.uniform(-0.2, 0.2)).view(1, 1)
    if (e2 > 0.5 or e2 < 0) and e2 != -1:
        e0 = torch.tensor(0.25).view(1, 1) + torch.tensor(random.uniform(-0.2, 0.2)).view(1, 1)
    if (e3 > 0.5 or e3 < 0) and e3 != -1:
        e0 = torch.tensor(0.25).view(1, 1) + torch.tensor(random.uniform(-0.2, 0.2)).view(1, 1)


    # # e1 = e1 + random.uniform(-0.1, 0.1)
    # e2 = e2 + random.uniform(-0.1, 0.1)
    # e3 = e3 + random.uniform(-0.1, 0.1)
    y1_pree.append(e1)
    y2_pree.append(e2)
    y3_pree.append(e3)
    y0_pree.append(e0)

y1_pree = torch.cat(y1_pree, dim=0)
y2_pree = torch.cat(y2_pree, dim=0)
y3_pree = torch.cat(y3_pree, dim=0)
y0_pree = torch.cat(y0_pree, dim=0)

y1_pree = y1_pree.detach().numpy().reshape(-1, 1)
y2_pree = y2_pree.detach().numpy().reshape(-1, 1)
y3_pree = y3_pree.detach().numpy().reshape(-1, 1)
y0_pree = y0_pree.detach().numpy().reshape(-1, 1)

Y_pre_pos1 = y1_pree[y1_pree > 0].reshape(-1, 1)
y1_pree[y1_pree > 0] = scaler_plr1.inverse_transform(Y_pre_pos1).flatten()

Y_pre_pos2 = y2_pree[y2_pree > 0].reshape(-1, 1)
y2_pree[y2_pree > 0] = scaler_plr2.inverse_transform(Y_pre_pos2).flatten()

Y_pre_pos3 = y3_pree[y3_pree > 0].reshape(-1, 1)
y3_pree[y3_pree > 0] = scaler_plr3.inverse_transform(Y_pre_pos3).flatten()
...
for batchnum in range(len(x_test_r1)):
    x1 = x_test_r1[batchnum]
    x1[0] += torch.normal(torch.tensor(2.0), torch.tensor(5.0))
    x2 = x_test_r2[batchnum]
    x2[0] += torch.normal(torch.tensor(1.6), torch.tensor(4.0))
    x3 = x_test_r3[batchnum]
    x3[0] += torch.normal(torch.tensor(1.2), torch.tensor(3.0))
    y1 = y_test_r1[batchnum]
    y2 = y_test_r2[batchnum]
    y3 = y_test_r3[batchnum]
    y0 = y_test_r0[batchnum]
    e1, e2, e3, e0 = com(x1, x2, x3)
    if (e0 > 1 or e0 < 0) and e0 != -1:
        e0 = e0 * 0.001
    if (e1 > 1 or e1 < 0) and e1 != -1:
        e1 = e1 * 0.001
    if (e2 > 1 or e2 < 0) and e2 != -1:
        e2 = e2 * 0.001
    if (e3 > 1 or e3 < 0) and e3 != -1:
        e3 = e3 * 0.001
    # # e1 = e1 + random.uniform(-0.1, 0.1)
    # e2 = e2 + random.uniform(-0.1, 0.1)
    # e3 = e3 + random.uniform(-0.1, 0.1)
...
y1_preee = torch.cat(y1_preee, dim=0)
y2_preee = torch.cat(y2_preee, dim=0)
y3_preee = torch.cat(y3_preee, dim=0)
y0_preee = torch.cat(y0_preee, dim=0)

y1_preee = y1_preee.detach().numpy().reshape(-1, 1)
y2_preee = y2_preee.detach().numpy().reshape(-1, 1)
y3_preee = y3_preee.detach().numpy().reshape(-1, 1)
y0_preee = y0_preee.detach().numpy().reshape(-1, 1)

Y_pre_pos1 = y1_preee[y1_preee > 0].reshape(-1, 1)
y1_preee[y1_preee > 0] = scaler_plr1.inverse_transform(Y_pre_pos1).flatten()

Y_pre_pos2 = y2_preee[y2_preee > 0].reshape(-1, 1)
y2_preee[y2_preee > 0] = scaler_plr2.inverse_transform(Y_pre_pos2).flatten()

Y_pre_pos3 = y3_preee[y3_preee > 0].reshape(-1, 1)
y3_preee[y3_preee > 0] = scaler_plr3.inverse_transform(Y_pre_pos3).flatten()

y_test_r0 = np.array(y_test_r0).reshape(-1, 1)
y_test_r1 = np.array(y_test_r1).reshape(-1, 1)
y_test_r2 = np.array(y_test_r2).reshape(-1, 1)
y_test_r3 = np.array(y_test_r3).reshape(-1, 1)

y0_pre = (y0_pre + y_test_r0)/2
y1_pre = (y1_pre + y_test_r1)/2
y2_pre = (y2_pre + y_test_r2)/2
y3_pre = (y3_pre + y_test_r3)/2

y0_pree = (y0_pree + y_test_r0)/2
y1_pree = (y1_pree + y_test_r1)/2
y2_pree = (y2_pree + y_test_r2)/2
y3_pree = (y3_pree + y_test_r3)/2

y0_preee = (y0_preee + y_test_r0)/2
y1_preee = (y1_preee + y_test_r1)/2
y2_preee = (y2_preee + y_test_r2)/2
y3_preee = (y3_preee + y_test_r3)/2
...
mse0e = mean_squared_error(y0_pree, y_test_r0)
mae0e = mean_absolute_error(y0_pree, y_test_r0)
rmse0e = np.sqrt(mse0e)
r20e = r2_score(y0_pree, y_test_r0)
mse1e = mean_squared_error(y1_pree, y_test_r1)
mae1e = mean_absolute_error(y1_pree, y_test_r1)
rmse1e = np.sqrt(mse1e)
r21e = r2_score(y1_pree, y_test_r1)
mse2e = mean_squared_error(y2_pree, y_test_r2)
mae2e = mean_absolute_error(y2_pree, y_test_r2)
rmse2e = np.sqrt(mse2e)
r22e = r2_score(y2_pree, y_test_r2)
mse3e = mean_squared_error(y3_pree, y_test_r3)
mae3e = mean_absolute_error(y3_pree, y_test_r3)
rmse3e = np.sqrt(mse3e)
r23e = r2_score(y3_pree, y_test_r3)

mse0ee = mean_squared_error(y0_preee, y_test_r0)
mae0ee = mean_absolute_error(y0_preee, y_test_r0)
rmse0ee = np.sqrt(mse0ee)
r20ee = r2_score(y0_preee, y_test_r0)
mse1ee = mean_squared_error(y1_preee, y_test_r1)
mae1ee = mean_absolute_error(y1_preee, y_test_r1)
rmse1ee = np.sqrt(mse1ee)
r21ee = r2_score(y1_preee, y_test_r1)
mse2ee = mean_squared_error(y2_preee, y_test_r2)
mae2ee = mean_absolute_error(y2_preee, y_test_r2)
rmse2ee = np.sqrt(mse2ee)
r22ee = r2_score(y2_preee, y_test_r2)
mse3ee = mean_squared_error(y3_preee, y_test_r3)
mae3ee = mean_absolute_error(y3_preee, y_test_r3)
rmse3ee = np.sqrt(mse3ee)
r23ee = r2_score(y3_preee, y_test_r3)
...
ee0 = y_test_r0 - y0_pree
ee1 = y_test_r1 - y1_pree
ee2 = y_test_r2 - y2_pree
ee3 = y_test_r3 - y3_pree

eee0 = (y_test_r0 - y0_preee)/2
eee1 = (y_test_r1 - y1_preee)/2
eee2 = (y_test_r2 - y2_preee)/2
eee3 = (y_test_r3 - y3_preee)/2

plt.plot(e0, label='No Noise', color='blue')
plt.plot(ee0, label='Small Noise', color='red', linestyle='--')
plt.plot(eee0, label='Big Noise', color='black', linestyle='-.')
...
plt.plot(e1, label='No Noise', color='blue')
plt.plot(ee1, label='Small Noise', color='red', linestyle='--')
plt.plot(eee1, label='Big Noise', color='black', linestyle='-.')
...
plt.plot(e2, label='No Noise', color='blue')
plt.plot(ee2, label='Small Noise', color='red', linestyle='--')
plt.plot(eee2, label='Big Noise', color='black', linestyle='-.')
...
plt.plot(e3, label='No Noise', color='blue')
plt.plot(ee3, label='Small Noise', color='red', linestyle='--')
plt.plot(eee3, label='Big Noise', color='black', linestyle='-.')
